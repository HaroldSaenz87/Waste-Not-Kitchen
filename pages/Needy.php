...
include '../dbs.project/config.php';
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['plate_id'])){
        ...
        $availQ = "SELECT SUM(oi.amt) - COALESCE(nr.reserved_count, 0) AS available_amt
                   FROM order_items oi
                   JOIN orders o ON oi.order_id = o.order_id AND o.order_type = 'donation'
                   LEFT JOIN(
                        SELECT plate_id, COUNT(*) AS reserved_count
                        FROM needy_reservations
                        GROUP BY plate_id
                    ) nr ON nr.plate_id = oi.plate_id
                    WHERE oi.plate_id = ? GROUP BY oi.plate_id
                    LIMIT 1";
